...
import org.kvn.checklyinn.server.dto.BookingResponse
import org.kvn.checklyinn.server.models.BookingStatus
    ...
    suspend fun createBooking(
        ...
        val calculatedPrice = totalPrice ?: run {
            ...
            if (checkIn != null && checkOut != null && listing[TravelListings.category] == "HOTEL") {
                val nights =
                    ((checkOut.toEpochMilliseconds() - checkIn.toEpochMilliseconds()) / (1000 * 60 * 60 * 24)).toInt()
                basePrice.multiply(BigDecimal.valueOf(nights.toLong()))
                    .multiply(BigDecimal.valueOf(numberOfGuests.toLong()))
            } else {
